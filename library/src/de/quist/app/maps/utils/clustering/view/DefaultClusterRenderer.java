...
import de.quist.app.maps.utils.R;
...
import de.quist.app.maps.Projection;
...
import de.quist.app.maps.utils.MarkerManager;
...
import de.quist.app.maps.utils.ui.SquareTextView;

import static de.quist.app.maps.utils.clustering.algo.NonHierarchicalDistanceBasedAlgorithm.MAX_DISTANCE_AT_ZOOM;
...
public class DefaultClusterRenderer<T extends ClusterItem> implements ClusterRenderer<T> {
    ...
    private final MapsBinding mMapsBinding;
     ...
     */
    private java.util.Map<Marker, Cluster<T>> mMarkerToCluster = new HashMap<Marker, Cluster<T>>();
    private java.util.Map<Cluster<T>, Marker> mClusterToMarker = new HashMap<Cluster<T>, Marker>();
    ...
    public DefaultClusterRenderer(Context context, MapsBinding mapsBinding, Map map, ClusterManager<T> clusterManager) {
        mMapsBinding = mapsBinding;
        mMap = map;
    ...
    @Override
    public void onAdd() {
        mClusterManager.getMarkerCollection().setOnMarkerClickListener(new Map.OnMarkerClickListener() {
            @Override
        ...
        mClusterManager.getMarkerCollection().setOnInfoWindowClickListener(new Map.OnInfoWindowClickListener() {
            @Override
        ...
        mClusterManager.getClusterMarkerCollection().setOnMarkerClickListener(new Map.OnMarkerClickListener() {
            @Override
            public boolean onMarkerClick(Marker marker) {
        ...
        mClusterManager.getClusterMarkerCollection().setOnInfoWindowClickListener(new Map.OnInfoWindowClickListener() {
            @Override
            public void onInfoWindowClick(Marker marker) {
        ...
        public void run() {
            ...
            for (Cluster<T> c : clusters) {
                ...
                if (zoomingIn && onScreen && SHOULD_ANIMATE) {
                    ...
                    if (closest != null) {
                        LatLng animateTo = mSphericalMercatorProjection.toLatLng(mMapsBinding, closest);
    ...
    private static class MarkerCache<T> {
        private java.util.Map<T, Marker> mCache = new HashMap<T, Marker>();
        private java.util.Map<Marker, T> mCacheReverse = new HashMap<Marker, T>();
    ...
    protected void onBeforeClusterRendered(Cluster<T> cluster, MarkerOptions markerOptions) {
        ...
        if (descriptor == null) {
            ...
            descriptor = mMapsBinding.bitmapDescriptorFactory().fromBitmap(mIconGenerator.makeIcon(getClusterText(bucket)));
        ...
        private void perform(MarkerModifier markerModifier) {
            ...
            if (!shouldRenderAsCluster(cluster)) {
                for (T item : cluster.getItems()) {
                    ...
                    if (marker == null) {
                        MarkerOptions markerOptions = mMapsBinding.newMarkerOptions();
                        if (animateFrom != null) {
    ...
    private class AnimationTask extends AnimatorListenerAdapter implements ValueAnimator.AnimatorUpdateListener {
        ...
        public void onAnimationUpdate(ValueAnimator valueAnimator) {
            ...
            LatLng position = mMapsBinding.newLatLng(lat, lng);
