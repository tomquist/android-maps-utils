...
public class PolyUtilTest extends TestCase {
    ...
    private static List<LatLng> makeList(double... coords) {
        ...
        for (int i = 0; i < size; ++i) {
            list.add(GoogleMapsBinding.INSTANCE.newLatLng(coords[i + i], coords[i + i + 1]));
        }
        return list;
    }
    ...
    public void testDecodePath() {
        List<LatLng> latLngs = PolyUtil.decode(GoogleMapsBinding.INSTANCE, TEST_LINE);
    ...
    public void testEncodePath() {
        List<LatLng> path = PolyUtil.decode(GoogleMapsBinding.INSTANCE, TEST_LINE);
